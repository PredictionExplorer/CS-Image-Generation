...
fn same_seed_produces_identical_borda_selection() {
    ...
    let (bodies1, traj1) = sim::select_best_trajectory(&mut rng1, 500, 5_000, 1.0, 5.0, -0.3)
        .expect("borda selection should succeed");
    let (bodies2, traj2) = sim::select_best_trajectory(&mut rng2, 500, 5_000, 1.0, 5.0, -0.3)
        .expect("borda selection should succeed");
...
fn same_seed_produces_identical_simulation() {
    ...
    let (bodies, _) = sim::select_best_trajectory(&mut rng, 200, 2_000, 1.0, 5.0, -0.3)
        .expect("borda selection should succeed");
...
fn different_seeds_produce_different_orbits() {
    ...
    let (bodies_a, _) = sim::select_best_trajectory(&mut rng_a, 200, 2_000, 1.0, 5.0, -0.3)
        .expect("borda selection should succeed");
    let (bodies_b, _) = sim::select_best_trajectory(&mut rng_b, 200, 2_000, 1.0, 5.0, -0.3)
        .expect("borda selection should succeed");
