...
struct Args {
    ...
    #[arg(long, default_value_t = false, help = "Skip generating extra outputs (loop clip, genesis burst, spectral gallery, phase portrait, sonification, avatar, wallpapers, stats card, reveal video, 3D model)")]
    no_extras: bool,
...
mod tests {
    ...
    #[test]
    fn test_extras_enabled_by_default() {
        let args = Args::parse_from(["three_body_problem"]);
        assert!(!args.no_extras, "extras should be generated by default");
    }

    #[test]
    fn test_no_extras_flag_disables_extras() {
        let args = Args::parse_from(["three_body_problem", "--no-extras"]);
        assert!(args.no_extras, "--no-extras should disable extras");
    }

    #[test]
    fn test_no_extras_combined_with_other_flags() {
        let args = Args::parse_from([
            "three_body_problem",
            "--no-extras",
            "--fast-encode",
