...
mod tests {
    ...
    fn test_prism_video_smoke() {
        ...
        let dir = std::env::temp_dir().join("spectral_test_prism");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("prism.mp4");
        let result = render_spectral_prism_video(&bins, path.to_str().unwrap(), true);
        assert!(result.is_ok(), "prism video render failed: {:?}", result.err());
    }
}
