...
void main() {
    ...
    float x=max(0.0,1.0-p.x*p.x/(u_aspect*u_aspect*u_domain*u_domain));
    float y=max(0.0,1.0-p.y*p.y/(u_domain*u_domain));
    float boundary=x*x*y*y;
    vec2 curl_boundary=vec2(-4.0*p.y*x*x*y,4.0*p.x*x*y*y/(u_aspect*u_aspect))/(u_domain*u_domain);
