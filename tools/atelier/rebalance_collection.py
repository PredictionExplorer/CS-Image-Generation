...
SPECS = (
    Spec(
        ...
        5642594,
        112,
        "2f707da3d6da01d564ab763fd17cb0cd2bfc7d4996eeee19372986290c9a847d",
    ),
    Spec(
        "04-light",
        15,
        ...
        6158625,
        32,
        "0bb2ad19e86bb38ee01804c1d1e89bb6d9516b677f9071060eb7ffd69e119439",
    ),
    Spec(
        "05-engraving",
        17,
        ...
        7236089,
        48,
        "34377c3aaad89823e200f9e081acfe7bf2e30c78bc73a2116a35aaf407b165d2",
    ),
    Spec(
        "06-eclipse",
        22,
        ...
        7367852,
        64,
        "3f8756738359380d4c9467d8a083482d335488be10e72acb8e54232abf5972ed",
    ),
)
...
class Processes:
    ...
    def environment(process):
        ...
        return {
            ...
            for key, value in (item.split(b"=", 1) for item in values if item)
        }

    @staticmethod
    def output_path(process, descriptor):
        return str((Path("/proc") / str(process.pid) / "fd" / str(descriptor)).readlink())
...
def same_process(current, identity, check_parent=True):
    ...
    if (
        any(getattr(current, key) != identity[key] for key in keys)
        or list(current.argv) != identity["argv"]
    ):
...
# Keep the original single, import-time default object; Paths is immutable.
DEFAULT_PATHS = Paths()
...
class Collection:
        ...
        self.known_hashes = (
            ...
            if known_hashes is not None
                ...
                **{str(paths.scripts / name): value for name, value in SCRIPT_HASHES.items()},
                str(paths.python): PYTHON_HASH,
                str(paths.orbit): ORBIT_HASH,
    ...
    def parse_driver(self, spec, command):
        if (
            ...
            or len(command) < 2
            or command[:2] != ["python3", str(self.paths.scripts / "render_study.py")]
        ):
        ...
        expected = {
            "--orbit": str(self.paths.orbit),
            "--config": str(self.paths.config(spec)),
            "--output": str(self.paths.output(spec)),
            "--executable": str(self.paths.binary(spec)),
            "--chunks": "16",
            "--parallel-ranges": "4",
            "--poster-frame": "900",
        }
    ...
    def job(self, spec):
        ...
        if (
            recipe != read_json(root / "render-request.json")
            or recipe.get("frames") != 1802
            or recipe.get("kind") != spec.slug.split("-", 1)[1]
        ):
    ...
    def complete(self, job):
        ...
        for index in range(16):
            ...
            if (
                manifest.get("config") != job.recipe
                or manifest.get("seed") != "0xb7f327f9f722"
    ...
    def driver_process(self, job, process, command):
        ...
        if (
            process.uid != self.proc.uid
            or process.exe != str(self.paths.python)
            or (process.device, process.inode) != (image.st_dev, image.st_ino)
            or list(process.argv) != command
    ...
    def renderer_command(self, job, index):
        return [
            ...
            "--config",
            str(self.paths.output(job.spec) / "render-request.json"),
            "--output",
            str(self.paths.output(job.spec) / f"final-chunk-{index}"),
            "--start",
            str(1802 * index // 16),
            "--end",
            str(1802 * (index + 1) // 16),
    ...
    def renderer(self, job, process, parent=None):
        ...
        if (
            process.uid != self.proc.uid
            or process.exe != str(self.paths.binary(job.spec))
            or (process.device, process.inode) != (image.st_dev, image.st_ino)
            or process.cwd != str(self.paths.base.parent)
        ):
            raise UnsafeJob(f"Renderer executable/cwd mismatch at PID {process.pid}")
        matches = [
            index for index in range(16) if list(process.argv) == self.renderer_command(job, index)
        ]
        if len(matches) != 1:
    ...
    def finishing_child(self, job, process):
        prefix = [
            ...
            str(self.paths.scripts / "finish_study.py"),
            "--output",
            str(self.paths.output(job.spec)),
            "--executable",
            str(self.paths.binary(job.spec)),
            "--poster-frame",
            "900",
        ...
        expected = prefix + [
            part
            for index in range(16)
            for part in ("--input", str(self.paths.output(job.spec) / f"final-chunk-{index}"))
        ]
        return (
            process.exe == str(self.paths.python)
            and bool(process.argv)
            and process.argv[0]
            in {str(self.paths.python), str(self.paths.python.with_name("python3"))}
    ...
    def writers(self, job, processes):
        ...
        for process in processes:
            ...
            if (
                process.exe != executable and (not process.argv or process.argv[0] != executable)
            ) or "render" not in process.argv:
                continue
            outputs = [
                process.argv[i + 1]
                for i, value in enumerate(process.argv[:-1])
                if value == "--output"
            ]
            # Writer matching is lexical. Resolving symlinks would change which
            # processes are treated as writers of the authorized output paths.
            if any(
                os.path.abspath(os.path.join(process.cwd, value)).startswith(root)  # noqa: PTH100, PTH118
...
def allocations(complete, current):
    ...
    pairs = {
        frozenset(("04-light", "06-eclipse")): {"04-light": 64, "06-eclipse": 64},
...
class Controller:
    ...
    def event(self, kind, **details):
        ...
        descriptor = os.open(
            self.collection.paths.private / "events.jsonl",
    ...
    def begin(self, job, workers):
        ...
        transaction = {
            ...
            "stage": "prepared",
            "old": driver.identity(),
            "children": [value.identity() for value in children],
            "old_command": job.command,
            "new_command": changed_workers(job.command, workers),
            "target_workers": workers,
            "record": job.record,
            "environment": self.collection.proc.environment(driver),
        }
    ...
    def recover(self):
        ...
        job = Job(
            spec,
            transaction["record"],
            transaction["old_command"],
            self.collection.parse_driver(spec, transaction["old_command"]),
            read_json(self.collection.paths.config(spec)),
        )
        self.collection.parse_driver(spec, transaction["new_command"])
        if (
            transaction["new_command"]
            != changed_workers(transaction["old_command"], transaction["target_workers"])
        ...
        candidates = [
            value
            for value in self.collection.proc.all()
            if list(value.argv) == transaction["new_command"]
        ]
        if candidates:
            if (
                len(candidates) != 1
                or self.collection.proc.read(transaction["old"]["pid"]) is not None
            ):
    ...
    def finish_transaction(self):
        ...
        try:
            ...
            if old is not None:
                ...
                if not self.collection.proc.wait(
                    [driver_pin], self.remaining(self.grace), stopped=True
                ):
                ...
                children = self.collection.children(
                    job, frozen
                )  # Include newly queued direct children.
            else:
                children = []
                ...
                if {
                    value.pid for value in self.collection.writers(job, self.collection.proc.all())
                } != {value.pid for value in children}:
            ...
            self.save()
            self.remaining()
            process = self.collection.proc.spawn(
                transaction["new_command"],
                transaction["old"]["cwd"],
                transaction["environment"],
                self.collection.paths.log(spec),
                self.collection.paths.python,
            )
            self.collection.driver_process(job, process, transaction["new_command"])
            ...
            if (
                ...
                and not self.collection.proc.exited(driver_pin)
            ):
                self.collection.proc.signal(driver_pin, signal.SIGCONT)
                self.state["transaction"] = None
                self.save()
...
def bootstrap(collection, timeout_hours):
    ...
    state = {
        "schema": 1,
        "boot_id": collection.proc.boot,
        "started_unix": now,
        "deadline_unix": now + timeout_hours * 3600,
        "pins": pins,
        "jobs": {},
        "transaction": None,
    }
    for name, spec in collection.specs.items():
        ...
        if not collection.complete(job):
            ...
            if (
                process.pid != spec.initial_pid
                or process.start != spec.initial_start
                or job.workers != spec.initial_workers
            ):
                raise UnsafeJob(f"Initial process changed since authorization: {name}")
            with contextlib.suppress(Deferred):
...
def validate_state(state, collection):
    if (
        state.get("schema") != 1
        or state.get("boot_id") != collection.proc.boot
    ...
    if (
        not all(isinstance(value, (int, float)) and math.isfinite(value) for value in (start, end))
        or not 0 < end - start <= MAX_SECONDS
    ):
...
def dry_plan(controller):
    if controller.state.get("transaction"):
        ...
        return {
            "mode": "dry-run",
            "pending_transaction": {
                key: transaction[key] for key in ("slug", "stage", "target_workers")
            },
            "action": "recovery required before a new allocation plan",
            "signals": [],
        }
    jobs, complete, desired = controller.plan()
    return {
        "mode": "dry-run",
        "aurora_rendering_complete": "03-aurora" in complete,
        "deadline_unix": controller.state["deadline_unix"],
        "signals": [],
        "jobs": [
            {
                "slug": name,
                "pid": job.record["pid"],
                "rendering_complete": name in complete,
                "workers": job.workers,
                "desired_workers": desired.get(name),
                "action": "finishing-or-complete"
                if name in complete
                else "keep"
                if desired[name] == job.workers
                else "restart",
                "new_command": changed_workers(job.command, desired[name])
                if name in desired and desired[name] != job.workers
                else None,
            }
...
def controller_lock(directory):
    ...
    descriptor = os.open(
        directory / "controller.lock", os.O_RDWR | os.O_CREAT | os.O_CLOEXEC | os.O_NOFOLLOW, 0o600
    )
    try:
        try:
...
def main(argv=None):
    ...
    modes.add_argument(
        "--apply",
        action="store_true",
        help="explicitly authorize the already-scoped process restarts",
    )
    modes.add_argument(
        "--dry-run", action="store_true", help="validate and print a plan; this is the default"
    )
    parser.add_argument(
        "--once", action="store_true", help="with --apply, perform one allocation pass then exit"
    )
    ...
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="run local semantic tests without touching collection jobs",
    )
    ...
    if args.self_test:
        ...
        suite = unittest.defaultTestLoader.discover(
            str(Path(__file__).parent), pattern="test_rebalance_collection.py"
        )
        return 0 if unittest.TextTestRunner(verbosity=2).run(suite).wasSuccessful() else 1
    if (
        not all(
            math.isfinite(value) and value > 0
            for value in (args.timeout_hours, args.poll_seconds, args.grace_seconds)
        )
        or args.timeout_hours > 720
        or args.poll_seconds > 3600
        or args.grace_seconds > 60
    ):
        ...
        try:
            while True:
                ...
                status = (
                    ...
                    tuple((name, job.workers) for name, job in jobs.items()),
                )
                if status != previous:
                    controller.event(
                        "allocation_status",
                ...
                if len(complete) == len(collection.specs):
                    controller.event(
                        "rendering_complete", note="Finishing/encoding processes remain untouched"
                    )
                    return 0
                changes = [
                    (name, desired[name])
            ...
            controller.event(
                "controller_failed",
                error=str(error),
                transaction_pending=state.get("transaction") is not None,
            )
            raise
